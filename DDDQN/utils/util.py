...
def plot_loss_curve(x, loss, filename, lines=None):
    fig = plt.figure()
    ax = fig.add_subplot(111, label="1")
    ...
    ax.scatter(x, running_avg, color="C0")
    ax.set_xlabel("Steps", color="C0")
    ax.set_ylabel("Moving Average Loss", color="C0")
    ax.tick_params(axis="x", colors="C0")
    ax.tick_params(axis="y", colors="C0")
...
"""## Testing Environment
Output two type of observation, one (preprocessd and stacked frame) for the model, one (original) for video creating
"""


class RepeatActionAndMaxFrameEval(gym.Wrapper):
    # input: environment, repeat
    # init frame buffer as an array of zeros in shape 2 x the obs space
    def __init__(self, env=None, repeat=4, clip_reward=False, no_ops=0, fire_first=False):
        super(RepeatActionAndMaxFrameEval, self).__init__(env)
        self.repeat = repeat
        self.shape = env.observation_space.low.shape
        self.frame_buffer = np.zeros_like((2, self.shape))
        self.clip_reward = clip_reward
    ...
    def step(self, action):
        ...
        for i in range(self.repeat):
            (obs, obs2), reward, done, info = self.env.step(action)
            if self.clip_reward:
                reward = np.clip(np.array([reward]), -1, 1)[0]
            t_reward += reward
            idx = i % 2
        ...
        max_frame = np.maximum(self.frame_buffer[0], self.frame_buffer[1])
        return (max_frame, obs2), t_reward, done, info

    def reset(self):
        obs, obs2 = self.env.reset()
        no_ops = np.random.randint(self.no_ops) + 1 if self.no_ops > 0 else 0
        for _ in range(no_ops):
            _, _, done, _ = self.env.step(0)
            if done:
...
class StackFramesEval(gym.ObservationWrapper):
    # init the new obs space (gym.spaces.Box) low & high bounds as repeat of n_steps. These should have been defined for vizdooom
    # WE may not have to use boxes in ur final model. But we can keep it for now
    # Create a return a stack of observations
    def __init__(self, env, repeat):
        super(StackFramesEval, self).__init__(env)
        self.observation_space = gym.spaces.Box(
            env.observation_space.low.repeat(repeat, axis=0),
            env.observation_space.high.repeat(repeat, axis=0),
            dtype=np.float32,
    ...
    def reset(self):
        self.stack.clear()
        observation, obs2 = self.env.reset()
        for _ in range(self.stack.maxlen):
            self.stack.append(observation)

        return np.array(self.stack).reshape(self.observation_space.low.shape), obs2
...
def make_envEVAL(env_name, shape=(84, 84, 1), repeat=4, clip_rewards=False, no_ops=0, fire_first=False):
    env = gym.make(env_name)
    env.game.set_mode(Mode.ASYNC_PLAYER)
    env.game.init()

    env = PreprocessFrameEval(shape, env)
    env = RepeatActionAndMaxFrameEval(env, repeat, clip_rewards, no_ops, fire_first)

    env = StackFramesEval(env, repeat)
